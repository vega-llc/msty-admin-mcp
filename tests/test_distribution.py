...
def test_package_version_and_operator_commands_are_current():
    ...
    assert 'serverInfo.version == "1.1.0"' in installed_check


def test_release_tag_must_match_project_version():
    valid = subprocess.run(
        [sys.executable, str(ROOT / "scripts/validate_release_tag.py"), "v1.1.0"],
        cwd=ROOT,
        check=False,
        capture_output=True,
        text=True,
...
def test_release_metadata_contains_sbom_provenance_and_checksums(tmp_path):
    ...
    assert provenance["subject"]["version"] == "1.1.0"
